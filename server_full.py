...
def delete_image(artikelnr: str, filename: str, request: Request, bg: BackgroundTasks):
    artikelnr = str(artikelnr).strip()
    already_meta = _meta_path(artikelnr).exists()
    already_imgs = any(RAW_DIR.glob(f"{artikelnr}_*.jpg"))
    if (already_meta or already_imgs) and str(allow_existing) != "1":
    ...
    if ".." in filename or "/" in filename or "\" in filename:
        return JSONResponse({"ok": False, "error": "Ungültiger Dateiname"}, status_code=400)
    if not filename.startswith(f"{artikelnr}_") or not filename.lower().endswith(".jpg"):
        return JSONResponse({"ok": False, "error": "Datei passt nicht zur Artikelnummer"}, status_code=400)
    ...
    if remaining:
        bg.add_task(_run_meta_background, artikelnr)

    return {"ok": True, "deleted": filename, "remaining": [x.name for x in remaining]}
